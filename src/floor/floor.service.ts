...
@Injectable()
export class FloorServiceImp extends FloorService {
    constructor(
    ...
    async create(CreateFloorDto: CreateFloorDto, id?: string): Promise<Floor> {
    ...
    async updateFloor(
        ...
        try {
            ...
            let { max_apartment } = updateFloorDto;
    ...
    async paginate(options: IPaginationOptions<IPaginationMeta>) {
        const result = this.floorRepository
            .createQueryBuilder("floor")
            .leftJoinAndSelect("floor.building", "building")
